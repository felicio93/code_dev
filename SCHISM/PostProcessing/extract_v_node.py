...
srun -N 1 -n 80 python extract_v_node.py > mpi_output_moor.log 2>&1

or multi-node:
srun -N 2 -n 160 python extract_v_node.py > mpi_output_moor.log 2>&1

I recommend you use as many nodes needed for: -n > number of .nc output files


This script is used to extract T,S,U,V from native schism outputs for a list of nodes.
This script was created with the intest of comparing SCHISM profiles with mooring data.

Altough the data gathering runs in parallel the merge happens in the headnode (the merging process can be slow)
"""
...
def merge_ds(z_file,T_file,S_file,u_file,v_file,i,schism_idx,chunk_size,rank):
    ...
    try:
        ...
        U = xr.open_dataset(u_file,chunks={'time': chunk_size})
        V = xr.open_dataset(v_file,chunks={'time': chunk_size})
    ...
    dsu = U.horizontalVelX[:,schism_idx,:]
    dsv = V.horizontalVelY[:,schism_idx,:]

    merged_ds = xr.merge([dsz,dsT,dsS,dsu,dsv])
...
def main():
    ...
    ### Enter inputs here:
    run='R09b'
    rdir = f"/work2/noaa/nos-surge/felicioc/BeringSea/{run}/outputs/"  #r"/work2/noaa/nosofs/felicioc/BeringSea/R07/outputs/"
    wdir = r"/work2/noaa/nos-surge/felicioc/BeringSea/P10/mooring/outputs/"
    schism_idx = [1054772, 1054408,  989945,  793218]#[989944,1054772,793218] #idx of nodes to be extracted
    chunk_size = 100
    ###
    ...
    u_files = sorted([f'{rdir}/{i}' for i in os.listdir(rdir) if i.startswith('horizontalVelX_')], key=natural_sort_key)
    v_files = sorted([f'{rdir}/{i}' for i in os.listdir(rdir) if i.startswith('horizontalVelY_')], key=natural_sort_key)
    file_list = [(z_files[i], T_files[i], S_files[i],u_files[i],v_files[i], i) for i in range(0, len(z_files))]
    ...
    for files in files_to_process:
        ...
        merged_ds = merge_ds(files[0],files[1],files[2],files[3],files[4],files[5],schism_idx,chunk_size,rank)
        sys.stdout.flush()
    ...
    if rank == 0:
        ...
        final_merged_ds.to_netcdf(wdir+f"mooring_{run}.nc")
