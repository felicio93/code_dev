...
srun -N 1 -n 80 python 2by1_hs.py > mpi_output.log 2>&1

or multi-node:
srun -N 2 -n 130 python 2by1_hs.py > mpi_output.log 2>&1
...
def plot(file0,
    ...
    for hr_idx,t in enumerate(ts):
        ...
        ax.set_title(f'No Spec Bound SCHISM+WWM at {i_time}')  
        # ax.plot(187.258618, 52.266526, 'kx', markersize=16)  
        ...
        ax2.set_title(f'Multi Station File Spec Bound SCHISM+WWM at {i_time}')
        # ax2.plot(187.258618, 52.266526, 'kx', markersize=16)  
...
def main():
    ...
    run0='R09a'
    run1='R10a'
    rdir0 = f"/work2/noaa/nos-surge/felicioc/BeringSea/{run0}/outputs/"
    rdir = f"/work2/noaa/nos-surge/felicioc/BeringSea/{run1}/outputs/"
    interv=0.2
    vmin, vmax = 0,6
    wdir = r"/work2/noaa/nos-surge/felicioc/BeringSea/P10/fields_plots/Hs_R09a_R10a/"
    ###
    ...
    if rank == 0:
        create_video_from_images(wdir, f"movie_hs_{run0}_vs_{run1}.mp4")
