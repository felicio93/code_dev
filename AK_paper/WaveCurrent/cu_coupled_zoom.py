...
srun -N 1 -n 80 python cu_coupled.py > mpi_output.log 2>&1

or multi-node:
srun -N 2 -n 130 python cu_coupled.py > mpi_output.log 2>&1
...
def plot(file,
    ...
    try:
        ...
        ts = np.array(ds0.time)

        dsX0 = xr.open_dataset(X_files0,
                            engine='h5netcdf',
                            drop_variables=['minimum_depth',
                                            'SCHISM_hgrid',
                                            'crs',
        ...
        vel0 = np.array(dsX0.horizontalVelX[:,:,-1]**2 + dsY0.horizontalVelY[:,:,-1]**2) ** 0.5
        del ds0,dsX0,dsY0

        dsX = xr.open_dataset(X_files,
                            engine='h5netcdf',
                            drop_variables=['minimum_depth',
                                            'SCHISM_hgrid',
                                            'crs',
        ...
        vel = np.array(dsX.horizontalVelX[:,:,-1]**2 + dsY.horizontalVelY[:,:,-1]**2) ** 0.5
        del dsX,dsY
    ...
    for hr_idx,t in enumerate(ts):
        ...
        tp = ax.tripcolor(triangulation,vel0[hr_idx],shading='flat',cmap=cmap,norm=norm)
        ...
        cb.set_label("Current Velocity (m/s): Uncoupled Model")

        ax.set_xlim(185.7,191.5)
        ax.set_ylim(51.75,53.25)
        ax.set_facecolor('gray')
        ax.set_title(f'Current Velocity (m/s) at {i_time}')  


        ax2 = fig.add_subplot(212,)
        tp2 = ax2.tripcolor(triangulation,((vel[hr_idx]-vel0[hr_idx])/vel0[hr_idx])*100,shading='flat',cmap='RdBu_r',vmin=-50,vmax=50)
        div2 = make_axes_locatable(ax2)
        cax2 = div2.append_axes("right", size="5%", pad=0.2)
        cb2 = fig.colorbar(tp2,
                      cax=cax2,
                      )
        cb2.set_label("% Change in Cu_Vel: (Coupled-Uncoupled)/Uncoupled")           
...
def main():
    ...
    interv=0.1
    vmin, vmax = 0,3
    wdir = r"/work2/noaa/nos-surge/felicioc/BeringSea/P09/cu_fig_coup_a_b_zoom/"
    ###
    ...
    # surface_files0 = sorted([f'{rdir0}/{i}' for i in os.listdir(rdir0) if i.startswith('out2d_')])
    surface_files = sorted([f'{rdir}/{i}' for i in os.listdir(rdir) if i.startswith('out2d_')], key=natural_sort_key)
    X_files0 = sorted([f'{rdir0}/{i}' for i in os.listdir(rdir0) if i.startswith('horizontalVelX_')], key=natural_sort_key)
    Y_files0 = sorted([f'{rdir0}/{i}' for i in os.listdir(rdir0) if i.startswith('horizontalVelY_')], key=natural_sort_key) 
    X_files = sorted([f'{rdir}/{i}' for i in os.listdir(rdir) if i.startswith('horizontalVelX_')], key=natural_sort_key)
    Y_files = sorted([f'{rdir}/{i}' for i in os.listdir(rdir) if i.startswith('horizontalVelY_')], key=natural_sort_key)
    # file_list = [(surface_files[i], X_files[i], Y_files[i], i) for i in range(0, len(surface_files))]
    file_list = [(surface_files[i],X_files0[i], Y_files0[i],X_files[i], Y_files[i], i) for i in range(0, len(X_files0))]
    ...
    for files in files_to_process:
        ...
        plot(files[0],files[1],files[2],files[3],files[4],files[5], vmin, vmax, interv, x, y, triangulation, wdir, rank, depth, isobaths = [200,2000])
    
    comm.Barrier()

    if rank == 0:
        create_video_from_images(wdir, "movie_cuR09coup.mp4")
