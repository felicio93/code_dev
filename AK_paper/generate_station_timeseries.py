...
df=pd.read_csv("ak_3d_staout_nc.csv", index_col=[0], sep=',',dtype=str)
...
# (2022/06/14)
        ...
        if ivar==0:
            ...
            for i in np.arange(nstation):
                names[i]=str(station_info[i+1])
            
            namesc=stringtochar(names)
            #namesc=np.char.array(names, itemsize=names.dtype.itemsize)
