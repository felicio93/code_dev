...
def merge_ds(z_file,T_file,S_file,i,schism_idx,rank,wdir):

    try:
        z = xr.open_dataset(z_file)
        T = xr.open_dataset(T_file)
    ...
    merged_ds.to_netcdf(wdir+f"{i:03}_merged.nc")
...
def main():
    ...
    wdir = r"/work2/noaa/nos-surge/felicioc/BeringSea/P09/mooring_R07_nochunck/"#r"/work2/noaa/nos-surge/felicioc/BeringSea/P09/mooring_R09a/"
    ...
    for files in files_to_process:
        ...
        merged_ds = merge_ds(files[0],files[1],files[2],files[3],schism_idx,rank,wdir)
        sys.stdout.flush()
